...
本技能在项目存在 openspec 配置时，设计落盘到 openspec 制品；否则回退 docs 落盘。

- **检测**：`openspec` CLI 可用 且 项目存在 `openspec/config.yaml`（或 `openspec/` 根）
- **有 openspec**：设计落盘到 `openspec/changes/<name>/proposal.md` + `design.md`（见"设计之后-文档"）
- **无 openspec**：回退原 `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md`
...
6. **编写设计文档** — 以 openspec 制品落盘（`proposal.md` + `design.md`），详见"设计之后-文档"
...
digraph brainstorming {
    ...
    "用户审查规格?" -> "grilling 审问?" [label="批准"];
    "grilling 审问?" -> "调用 writing-plans 技能" [label="不审或审完"];
    "grilling 审问?" -> "回到设计" [label="审出盲点需改设计"];
}
```
...
- **openspec 落盘**（检测到 openspec 时）：
  ...
  5. `openspec validate <name>` — 格式兜底（需指定变更名，裸 validate 无参会进入交互选择）
- **回退 docs 落盘**（无 openspec 时）：保存到 `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md`（用户对规格位置的偏好优先于此默认值）
...
5. **openspec 格式校验（openspec 模式）：** 运行 `openspec validate <name>`，确保 proposal/design 符合制品格式规范。
...
> "设计已编写并 commit 到 `openspec/changes/<name>/design.md`（openspec 模式）或 `<docs 路径>`（回退模式）。请审查一下，如果在我们开始编写实现计划之前你想做任何修改，请告诉我。"

等待用户回复。如果他们要求修改，做出修改并重新运行规格自检。只有在用户批准后才继续。

**实现：**

- 在调用 writing-plans 之前，先询问用户：
  > "设计已经审查通过。在开始编写实现计划前，要不要先用 grilling 审问一轮？可以沿着决策树逐个分支审视，提前发现盲点。"
