...
本技能在项目存在 openspec 配置时，任务输入取自 openspec 制品、任务完成后归档；否则回退 docs 计划。

- **检测**：`openspec` CLI 可用 且 项目存在 `openspec/` 根
- **有 openspec**：`openspec instructions apply --change <name> --json` 拿 contextFiles（proposal/specs/design/tasks），子代理任务输入从 `tasks.md` 提取；全部完成后执行归档（确认 delta 同步意图：`openspec archive` 或 `--skip-specs`，见"openspec 归档"）
- **无 openspec**：回退读取 `docs/superpowers/plans/<plan>.md`
...
digraph process {
    ...
    "读取 openspec 制品（或回退计划），提取所有任务的完整文本，记录上下文，创建 TodoWrite" [shape=box];
    "还有剩余任务?" [shape=diamond];
    "分派最终代码审查子智能体审查整体实现" [shape=box];
    "openspec 归档（openspec 模式）" [shape=box];
    "使用 superpowers:finishing-a-development-branch" [shape=box style=filled fillcolor=lightgreen];

    "读取 openspec 制品（或回退计划），提取所有任务的完整文本，记录上下文，创建 TodoWrite" -> "分派实现子智能体 (./implementer-prompt.md)";
    ...
    "分派最终代码审查子智能体审查整体实现" -> "openspec 归档（openspec 模式）";
    "openspec 归档（openspec 模式）" -> "使用 superpowers:finishing-a-development-branch";
}
```
...
[一次性读取任务来源：openspec 模式 → `openspec instructions apply --change <name> --json` 拿 contextFiles（proposal/specs/design/tasks）并读取；回退模式 → docs/superpowers/plans/feature-plan.md]
[提取全部 5 个任务的完整文本和上下文]
[用所有任务创建 TodoWrite]
...
## openspec 归档（openspec 模式，所有任务完成后）

openspec 模式下，全部任务完成、最终代码审查通过后执行归档。**`openspec archive` 原生执行三件事：应用 delta 到主规范 + 默认校验 + 归档，无需手动合并主规范。**

1. `openspec status --change <name> --json` — 检查制品与任务完成度；制品未 done 或任务未完成时先列出警告并让用户确认（CLI 归档前默认校验任务完成度）
2. **确认 delta 同步意图**（不要手动编辑主规范）：
   - 同步（默认）：`openspec archive <name> --yes` — 自动把 `specs/<capability>/spec.md` 的 delta 应用到主规范 `openspec/specs/<capability>/spec.md`，默认校验，然后归档。**agent 非交互语境必须加 `--yes`**：归档前会提示 `Proceed with spec updates?`，无 TTY 时读不到 stdin 会直接失败
   - 不同步（纯基建/工具/仅文档变更）：`openspec archive <name> --skip-specs` — 只归档，不动主规范（免确认）
   - 询问用户选哪种后执行
3. 归档到 `openspec/changes/archive/YYYY-MM-DD-<name>/`

## 实现中设计问题 → 更新制品（openspec 模式）

实现子代理揭示设计与实现矛盾时（区别于单纯代码缺陷）：

- 暂停该任务，不继续实现
- 判断是制品遗漏（需求/设计没写全）还是实现偏离
- 若是制品问题：修订 `openspec/changes/<name>/` 下 proposal/design/specs/tasks，保持四者一致（**只改制品，不碰代码**——对应原 openspec-update-change 职责）
- 修订后回到子代理继续实现，并让规格审查覆盖修订点
