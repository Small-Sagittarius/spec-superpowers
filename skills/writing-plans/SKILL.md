...
- 有 openspec：计划落盘到 `openspec/changes/<name>/specs/<capability>/spec.md`（delta）+ `tasks.md`（见"规格化输出"）
- 无 openspec：回退 `docs/superpowers/plans/YYYY-MM-DD-<feature-name>.md`

**计划保存位置（回退 docs 模式）：** `docs/superpowers/plans/YYYY-MM-DD-<feature-name>.md`
- （用户对计划位置的偏好优先于此默认值）
...
openspec 模式下，计划产出的是两个 openspec 制品，而非单一文档：

**specs delta**（`openspec/changes/<name>/specs/<capability>/spec.md`）——需求增量，格式：
...
**tasks.md**（`openspec/changes/<name>/tasks.md`）——实现任务，`- [ ]` 复选框格式（结构见"任务结构"）。

生成步骤：
1. `openspec status --change <name> --json` — 衔接 brainstorming 的 design done，确认 specs/tasks 依赖为 ready
2. `openspec instructions specs --change <name> --json` + `openspec instructions tasks --change <name> --json` — 取模板、规则、依赖
3. 按模板：把设计中的需求组织成 specs delta（每条 Requirement 带可验证 Scenario），把实现步骤组织成 tasks.md
4. `openspec validate <name>` + `openspec status` — 确认 `applyRequires` 全部 done，进入实现

回退 docs 模式不需要此小节，计划直接保存为单一文档（见"计划保存位置"）。
...
**4. openspec 校验（openspec 模式）：** 运行 `openspec validate <name>` + `openspec status`，确认 specs delta 格式合规、`applyRequires` 全部 done。
...
> "计划已经完成自检。要不要用 grilling 技能来压力测试一下这个计划？可以沿着决策树的每个分支审视，检查是否有遗漏的边界情况或风险点。"

- 如果用户同意，调用 `Skill` 工具加载 `grilling` 技能执行
- 如果用户拒绝，继续下一步
- 审问结束后回到此处继续执行交接
...
**"计划已完成并保存到 `openspec/changes/<name>/tasks.md`（openspec 模式）或 `docs/superpowers/plans/<filename>.md`（回退模式）。两种执行方式：**
